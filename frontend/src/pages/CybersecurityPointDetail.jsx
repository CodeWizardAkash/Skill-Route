...
import { cybersecurityVisualsByPointId } from "../data/cybersecurityVisuals"
...
function getDefaultOpenSections(content, hasVisual) {
  const sectionIds = ["explanation"]

  if (hasVisual) {
    sectionIds.push("visual")
  }
  ...
  if (content?.resources?.length) {
    sectionIds.push("resources")
  }

  return sectionIds
}

export default function CybersecurityPointDetail() {
  const { topicId, pointId } = useParams()
  const content = cybersecurityData.topicsContent[pointId]
  const visual = cybersecurityVisualsByPointId[pointId]
  const [openSectionIds, setOpenSectionIds] = useState(() => getDefaultOpenSections(content, Boolean(visual)))
  ...
  if (!content) {
    return (
      <div className="p-4 md:p-10 text-gray-900 dark:text-white transition-colors">
        ...
        <Link to={`/cyber-security/${topicId}`} className="text-cyan-500 dark:text-cyan-300 hover:underline mt-4 inline-block transition-colors">
          Return to Cyber Security roadmap
        </Link>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-10 text-gray-900 dark:text-white transition-colors">
      ...
      >
        <span className="mr-2">&lt;-</span> Back to Cyber Security roadmap
      </Link>
        ...
        <div className="space-y-4">
          ...
          {content.code && (
            <section className="rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/30 transition-colors">
              <button
                ...
                className="w-full flex items-center justify-between gap-4 px-4 py-3 text-left"
              >
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white transition-colors">Command Example</h2>
                <span className="shrink-0 rounded-md border border-gray-300 dark:border-gray-600 px-2 py-1 text-xs font-semibold text-gray-600 dark:text-gray-300 transition-colors">
          ...
          {content.resources?.length > 0 && (
            <section className="rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/30 transition-colors">
              <button
                type="button"
                onClick={() => toggleOpenId("resources", setOpenSectionIds)}
                aria-expanded={isSectionOpen("resources")}
                className="w-full flex items-center justify-between gap-4 px-4 py-3 text-left"
              >
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white transition-colors">Learning Resources</h2>
                <span className="shrink-0 rounded-md border border-gray-300 dark:border-gray-600 px-2 py-1 text-xs font-semibold text-gray-600 dark:text-gray-300 transition-colors">
                  {isSectionOpen("resources") ? "Hide" : "Show"}
                </span>
              </button>

              {isSectionOpen("resources") && (
                <div className="px-4 pb-4">
                  <ul className="space-y-2 text-sm md:text-base text-gray-700 dark:text-gray-300 transition-colors">
                    {content.resources.map(resource => (
                      <li key={resource.url} className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900/40 px-3 py-2">
                        <a
                          href={resource.url}
                          target="_blank"
                          rel="noreferrer"
